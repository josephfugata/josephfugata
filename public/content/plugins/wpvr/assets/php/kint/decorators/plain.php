...
class Kint_Decorators_Plain
{
	...
	public static function decorate( kintVariableData $kintVar, $level = 0 )
	{
		...
		if ( $kintVar->extendedValue !== null ) {
			...
			if ( is_array( $kintVar->extendedValue ) ) {
				foreach ( (array) $kintVar->extendedValue as $v ) {
	...
	public static function decorateTrace( $traceData )
	{
		...
		foreach ( (array) $traceData as $stepNo => $step ) {
			...
			if ( !empty( $step['args'] ) ) {
				...
				$i = 0;
				foreach ( (array) $step['args'] as $name => $argument ) {
					$argument           = kintParser::factory(
						$argument,
						$name ? $appendDollar . $name : '#' . ++$i
					);
					$argument->operator = $name ? ' =' : ':';
					$maxLevels          = wpvrKint::$maxLevels;
					if ( $maxLevels ) {
						wpvrKint::$maxLevels = $maxLevels + 2;
					}
					$output .= self::decorate( $argument, 2 );
					if ( $maxLevels ) {
						wpvrKint::$maxLevels = $maxLevels;
					}
				}
				$output .= '    )' . PHP_EOL;
			}
			...
			if ( !empty( $step['object'] ) ) {
				...
				if ( $maxLevels ) {
					...
					wpvrKint::$maxLevels = wpvrKint::enabled() === wpvrKint::MODE_CLI
						? 1
						: $maxLevels + 1;
				}
	...
	private static function _colorize( $text, $type, $nlAfter = true )
	{
		...
		switch ( wpvrKint::enabled() ) {
			case wpvrKint::MODE_PLAIN:
	...
	{
		if ( wpvrKint::enabled() === wpvrKint::MODE_PLAIN ) {
	...
	public static function wrapEnd( $callee, $miniTrace, $prevCaller )
	{
		...
		$lastChar = wpvrKint::enabled() === wpvrKint::MODE_PLAIN ? '</pre>' : '';


		if ( !wpvrKint::$displayCalledFrom ) return $lastLine . $lastChar;
	...
	{
		if ( wpvrKint::enabled() === wpvrKint::MODE_CLI ) { // todo win/nix
			return "+{$callee['line']} {$callee['file']}";
		}

		$url           = wpvrKint::getIdeLink( $callee['file'], $callee['line'] );
		$shortenedName = wpvrKint::shortenPath( $callee['file'] ) . ':' . $callee['line'];
	...
	public static function init()
	{
		...
		return wpvrKint::enabled() === wpvrKint::MODE_PLAIN
