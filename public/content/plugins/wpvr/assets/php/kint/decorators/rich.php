...
{
	# make calls to wpvrKint::dump() from different places in source coloured differently.
	...
	public static function decorate( kintVariableData $kintVar )
	{
		...
		if ( isset( $kintVar->extendedValue ) ) {
			...
			foreach ( (array) $kintVar->_alternatives as $k => $var ) {
			...
			foreach ( (array) $kintVar->_alternatives as $var ) {
				$output .= "<li>";
				...
				if ( is_array( $var ) ) {
					...
					throw new Exception(
						'wpvrKint has encountered an error, '
	...
	public static function decorateTrace( $traceData )
	{
		...
		foreach ( (array) $traceData as $i => $step ) {
			$class = 'kint-parent';
			if ( wpvrKint::$expandedByDefault ) {
				$class .= ' kint-show';
			}
			...
			if ( !empty( $step['args'] ) ) {
				$output .= "<li>";
				foreach ( (array) $step['args'] as $k => $arg ) {
					kintParser::reset();
	...
	/**
	 * closes wpvrKint::_wrapStart() started html tags and displays callee information
	 *
	 * @param array $callee caller information taken from debug backtrace
	...
	public static function wrapEnd( $callee, $miniTrace, $prevCaller )
	{
		...
		if ( !empty( $miniTrace ) ) {
			$traceDisplay = '<ol>';
			foreach ( (array) $miniTrace as $step ) {
	...
	{
		$shortenedPath = wpvrKint::shortenPath( $file );
		if ( !wpvrKint::$fileLinkFormat ) return $shortenedPath . ':' . $line;

		$ideLink = wpvrKint::getIdeLink( $file, $line );
	...
	public static function init()
	{
		...
		if ( !is_readable( $cssFile = $baseDir . wpvrKint::$theme . '.css' ) ) {
