...
class wpvr_cmb_Meta_Box {
	...
	public function save_post( $post_id, $post = false ) {
		...
		$override_cmb_nonce = apply_filters('wpvr_extend_override_cmb_nonce' , false , $post_id );
		
		// check permissions
		if (
			// check nonce
			! isset( $_POST['wp_meta_box_nonce'] )
			|| ( $override_cmb_nonce === false && ! wp_verify_nonce( $_POST['wp_meta_box_nonce'], self::nonce() ) )
			// check if autosave
		...
		{
			// d('Could not save fields ... ');
			// d(  self::nonce() );
			// d(  wp_verify_nonce( $_POST['wp_meta_box_nonce'], self::nonce() ) );
			return $post_id;
		}
