 ...
 * @since  0.0.4
 */
class wpvr_cmb_Meta_Box_Sanitize {

	/**
	 * A CMB field object
	 * @var wpvr_cmb_Meta_Box_field object
	 */
	public $field;
	...
	public function colorpicker( $value ) {
		...
		if ( is_array( $value ) ) {
			$check = $value;
			$value = array();
			foreach ( (array) $check as $key => $val ) {
				if ( $val && '#' != $val ) {
	...
	public function text_datetime_timestamp_timezone( $value, $repeat = false ) {
		...
		if ( empty( $tzstring ) )
			$tzstring = wpvr_cmb_Meta_Box::timezone_string();

		$offset = wpvr_cmb_Meta_Box::timezone_offset( $tzstring, true );
	...
	public function _save_file_id( $value ) {
		...
		$field      = new wpvr_cmb_Meta_Box_field( $args, $group );
		$id_key     = $field->_id();
		...
		if ( $value && ! $id_val ) {
			$id_val = wpvr_cmb_Meta_Box::image_id_from_url( $value );
		}
	...
	public function _check_repeat( $value, $method, $repeat ) {
		if ( $repeat || ! $this->field->args( 'repeatable' ) )
			return;
		$new_value = array();
		foreach ( (array) $value as $iterator => $val ) {
